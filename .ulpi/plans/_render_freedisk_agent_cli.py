#!/usr/bin/env python3
"""Canonical plan object -> .ulpi/plans/freedisk-agent-cli.{json,md}"""
from __future__ import annotations

import json
from pathlib import Path

PLAN_NAME = "freedisk-agent-cli"
OUT = Path(__file__).resolve().parent

plan = {
    "name": PLAN_NAME,
    "title": "Agent-friendly freedisk CLI",
    "mode": "EXPANSION",
    "review": "none",
    "overview": (
    ...
    "contracts": [
        {
            ...
            "detail": (
                ...
                "dev = volume + worktrees + artifacts + catalog paths tagged dev. "
        ...
        {
            "name": "Findings JSON",
            "detail": "Must validate against findings.schema.json. extra fields forbidden. reclaim.cmd is proposed text only.",
        },
        {
            "name": "Catalog merge",
            "detail": (
                "Bundled catalog/macos-hotspots.yaml + user overlay at "
                "$XDG_CONFIG_HOME/freedisk/catalog.yaml else ~/.config/freedisk/catalog.yaml (or --config). "
                "add[] appends paths with optional scans:[mode,...] (default quick,dev,full). "
                "Adding an already-known path unions scans. "
                "disable[] omits a path from every mode (unknown disable is a no-op). "
        ...
        {
            "name": "hgDB cleanup-space.sh learnings",
            "detail": (
                "External script /Users/ciprian/work_cip/holly_grail/hgDB/scripts/cleanup-space.sh. Encode the rules, do not copy the mutator. "
                "(1) Project artifacts vs shared caches: target/node_modules/.next/dist are rebuildable in-repo; ~/.cargo/registry and ~/.cargo/git affect every Rust project — risk ask, only as their own finding ids, never as a side effect of cleaning a repo. "
                "(2) Cargo target/ is often the largest reclaimable (hundreds of GB on a hot tree, and the expensive part of agent worktrees). Find target/ next to Cargo.toml at repo root, examples/*/target, sdks/*/target, and vendor/<crate>/target. Reclaim cmd: cargo clean in that tree. "
                "(3) rust vendor/ is SOURCE (vendor/paste/src). Do not classify cargo-vendor or Rails/Go vendor as Composer. Do not prune all vendor/ on the walk — only prune Composer vendor (parent composer.json); still record nested target/. Never propose deleting proptest-regressions/ or any git-tracked path. "
                "(4) JS extras beyond node_modules: .next, dist, .tsbuildinfo (file), .tsx-cache; also under examples/*/ and sdks/*/. "
                "(5) Worktrees: never rm -rf .claude/worktrees. Only leftover-worktree if git worktree list says prunable/gone AND older than worktree_inflight_hours (24), or dir mtime older than worktree_idle_days (14). Fresh agent checkouts are ask/keep. Reclaim: git worktree prune --expire 1.day or git worktree remove <path>. "
                "(6) /tmp/<project>-* test scratch is reclaimable only when the user (or overlay) listed a glob; do not wipe /tmp. catalog add supports --glob. Missing glob matches skip. "
...
def T(**kw):
    ...
    kw.setdefault("review", "none")
    kw.setdefault("filesToModify", [])
    kw.setdefault("filesToCreate", [])
    ws = kw.get("writeScope") or (kw["filesToCreate"] + kw["filesToModify"])
    kw["writeScope"] = ws
    return kw
...
plan["tasks"] = [
    T(
        id="TASK-001",
        title="Create Go module and version package",
        description=(
            "Initialize module path `github.com/CiprianSpiridon/free-disk-space` (this GitHub repo). Add internal/version "
            "with Version string default `0.0.0-dev` and a test that it is non-empty. No CLI yet."
        ),
        type="infra",
        priority="P0",
        effort="S",
        ...
        dependsOn=[],
        labels=["foundation"],
        filesToCreate=["go.mod", "internal/version/version.go", "internal/version/version_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-002",
        title="Findings types matching findings.schema.json",
        description=(
            "Add internal/findings types for the JSON document: Report, Host, Volume, Finding, Risk, Reclaim. "
            "Risk is a string enum matching findings.schema.json. JSON tags must match the schema property names. "
            "Do not execute reclaim commands here."
        ),
        type="feature",
        priority="P0",
        effort="S",
        ...
        dependsOn=["TASK-001"],
        labels=["contract"],
        filesToCreate=["internal/findings/types.go", "internal/findings/types_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-003",
        title="Load bundled catalog/macos-hotspots.yaml",
        description=(
    ...
    T(
        id="TASK-004",
        title="User catalog overlay merge",
        description=(
            "Document overlay schema in catalog/overlay.schema.yaml: add[] (path, optional risk, category, scans), "
            "disable[], unassign[] (path + from[]), disable_scans[], modes.<name>.types[], optional work_roots[]. "
            "Load from $XDG_CONFIG_HOME/freedisk/catalog.yaml else ~/.config/freedisk/catalog.yaml or --config. "
            "add appends (default scans quick,dev,full); add of an existing path unions scans. "
            "disable drops the path from every mode; unknown disable is a no-op. "
            "unassign subtracts listed modes only. ModePaths(mode) returns paths tagged for that mode. "
            "DisabledScans() returns disable_scans. Missing overlay file = bundled defaults only."
        ),
        type="feature",
        priority="P0",
        effort="M",
    ...
    T(
        id="TASK-006",
        title="Path policy (never system roots)",
        description=(
            "Absolute paths only; reject `..` as a component; reject control characters; deny prefixes from "
            "catalog thresholds.skip_system_prefixes (/System, /usr except /usr/local, /bin, /sbin, /private/var/vm, /dev, /net). "
            "Allow /usr/local, /tmp, /private/tmp (project scratch globs). "
            "Refuse deleting `$HOME/.cargo` as a whole (subpaths cache/src/git are decided at apply with risk). "
        ...
        dependsOn=["TASK-001"],
        labels=["safety"],
        filesToCreate=["internal/policy/path.go", "internal/policy/path_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-007",
        title="Volume inventory from diskutil fixtures",
        description=(
        ...
        dependsOn=["TASK-002"],
        labels=["scan"],
        filesToCreate=["internal/scan/volume.go", "internal/scan/volume_test.go"],
        acceptanceCriteria=[
    ...
    T(
        ...
        description=(
            ...
            "size existing with internal/size. "
            "Emit findings with category/risk/reclaim from the YAML. Do not recurse except later drill. "
        ...
        labels=["scan"],
        filesToCreate=["internal/scan/known.go", "internal/scan/known_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-009",
        title="CLI router (no business logic)",
        description=(
            "cmd/freedisk/main.go calls internal/cli.Execute. Root command `freedisk` with persistent flags "
            "--json, --catalog, --config. Subcommands register via cli.AddCommand from their own files later. "
            "Unknown command exits nonzero. Version from internal/version."
        ),
        type="feature",
        priority="P0",
        effort="S",
        ...
        dependsOn=["TASK-001"],
        labels=["cli"],
        filesToCreate=["cmd/freedisk/main.go", "internal/cli/root.go", "internal/cli/root_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-010",
        title="JSON report encoder",
        description=(
            "Encode findings.Report to JSON. Stable key order not required; required keys must exist; "
            "do not emit empty reclaim objects. Used by scan --json."
        ),
        type="feature",
        priority="P1",
        effort="S",
        ...
        dependsOn=["TASK-002"],
        labels=["report"],
        filesToCreate=["internal/report/json.go", "internal/report/json_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-012",
        title="scan command: --mode quick|dev|full|user, JSON/md, never deletes",
        description=(
            "internal/scan/run.go runs registered phases. Each Phase has Name plus Quick/Dev bools. "
            "`scan --mode=NAME` (default full); aliases `--quick` and `--dev`. "
        ...
        effort="M",
        agent="go-cli-senior-engineer",
        dependsOn=["TASK-007", "TASK-008", "TASK-009", "TASK-010", "TASK-011"],
        labels=["cli", "scan"],
        filesToCreate=["internal/cli/scan.go", "internal/scan/run.go", "internal/scan/run_test.go"],
        acceptanceCriteria=[
    ...
    T(
        ...
        effort="M",
        agent="go-cli-senior-engineer",
        dependsOn=["TASK-012", "TASK-005"],
        labels=["scan"],
        filesToCreate=["internal/scan/drill.go", "internal/scan/drill_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-015",
        title="Worktree leftovers (git, Claude, Grok)",
        description=(
            "Phase with Quick=false, Dev=true: size <repo>/.claude/worktrees, ~/.grok/worktrees, .ulpi/worktrees, git worktree list --porcelain. "
            "Missing roots append not_present, do not fail. Count field = number of child worktrees. "
            "Never emit reclaim `rm -rf` of the whole .claude/worktrees directory. "
            "leftover-worktree only if: git says prunable/gone AND mtime older than worktree_inflight_hours (24), "
            "OR dir mtime older than worktree_idle_days (14). Newer in-flight agent trees are ask or keep. "
            "reclaim.cmd: `git worktree prune --expire 1.day` or `git worktree remove <path>`. "
            "Participates in --dev and full, not --quick."
        ),
        type="feature",
        priority="P2",
        effort="M",
        agent="go-cli-senior-engineer",
        dependsOn=["TASK-012"],
        labels=["scan", "worktrees"],
        filesToCreate=["internal/scan/worktrees.go", "internal/scan/worktrees_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-016",
        title="Apple + Android simulator inventory",
        description=(
            "Phase with Quick=false, Dev=false (full scan only). Apple: parse simctl JSON fixtures; join devices to runtimes on runtimeIdentifier not display name. "
            "Android: size /opt/homebrew/share/android-commandlinetools if present in testdata overlay; zero AVDs still reports system-images as unused-runtime. "
            "which emulator missing is not 'SDK absent'."
        ),
        type="feature",
        priority="P2",
        effort="L",
    ...
    T(
        ...
        dependsOn=["TASK-012"],
        labels=["persist"],
        filesToCreate=["internal/scan/persist.go", "internal/scan/persist_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-018",
        title="catalog add/unassign/disable/list/path",
        description=(
            "Subcommand group that only edits the user overlay file. "
            "`catalog add PATH [--scans quick,dev,full] [--risk ask --category user] [--glob]` "
            "(default --scans quick,dev,full; add of a known path unions scans; --glob sets glob:true for /tmp/proj-* and similar). "
            "`catalog unassign PATH --from MODE[,MODE]`. "
            "`catalog disable PATH`. `catalog list [--json]`. `catalog path`. "
            "Creates ~/.config/freedisk/catalog.yaml (or --config) on first write. Does not scan or delete disk data. "
            "Long help must show add/unassign/disable and `--glob` examples."
        ),
        type="feature",
        priority="P1",
        effort="M",
        agent="go-cli-senior-engineer",
        dependsOn=["TASK-004", "TASK-009"],
        labels=["cli", "catalog"],
        filesToCreate=["internal/cli/catalog.go", "internal/cli/catalog_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-020",
        title="why: top reclaimable from last scan",
        description=(
            "`freedisk why [--json] [--limit N]` reads last-scan and prints the largest findings with risk "
            "safe-cache|rebuildable|leftover-worktree|unused-runtime|ask. Missing last scan: message, exit 0, no scan of $HOME unless --rescan is added (do not add --rescan in this task)."
        ),
        type="feature",
        priority="P2",
        effort="S",
        agent="go-cli-senior-engineer",
        dependsOn=["TASK-012", "TASK-017"],
        labels=["cli"],
        filesToCreate=["internal/cli/why.go", "internal/cli/why_test.go"],
        acceptanceCriteria=[
    ...
    T(
        id="TASK-021",
        title="Agent-facing AGENTS.md and README usage",
        description=(
        ...
        dependsOn=["TASK-012", "TASK-018", "TASK-019", "TASK-022", "TASK-023"],
        labels=["docs", "agents"],
    ...
    T(
        id="TASK-022",
        title="Agent-oriented help command",
        description=(
            "internal/cli/help.go registers `help [command]` and a custom root help function so `freedisk help` "
            "and `freedisk --help` print the same long agent guide. The guide MUST include these copy-paste lines: "
        ...
        effort="S",
        agent="go-cli-senior-engineer",
        dependsOn=["TASK-009", "TASK-012", "TASK-018", "TASK-023"],
    ...
    T(
        id="TASK-023",
        title="scans list/disable/enable/add/remove",
        description=(
            "Subcommand group that only edits overlay disable_scans and modes. Does not delete disk data. "
            "`scans list [--json]` prints built-in types (volume, known, drill, artifacts, worktrees, apple-sim, android-sim) "
            "with enabled bool, plus user modes. "
            "`scans disable TYPE` / `enable TYPE` (volume disable exits 2). "
            "`scans add NAME --types TYPE[,TYPE]` writes modes.NAME; NAME must not be quick|dev|full or a phase name. "
            "`scans remove NAME` deletes a user mode only (removing quick/dev/full/volume exits 2). "
            "After add, `scan --mode=NAME` runs those types plus volume. Long help must list the examples."
        ),
        type="feature",
        priority="P1",
        effort="M",
        agent="go-cli-senior-engineer",
        dependsOn=["TASK-004", "TASK-009", "TASK-012"],
        labels=["cli", "scans"],
        filesToCreate=["internal/cli/scans.go", "internal/cli/scans_test.go"],
        acceptanceCriteria=[
...
def layers(tasks):
    ...
    layer = 0
    while remaining:
        ready = [i for i in remaining if all(d not in remaining for d in by_id[i]["dependsOn"])]
        if not ready:
            raise SystemExit(f"cycle or missing dep: {remaining}")
        for i in ready:
            assigned[i] = layer
        remaining -= set(ready)
        layer += 1
...
lay, nlayers = layers(plan["tasks"])
cpath, clen = critical_path(plan["tasks"])
plan["execution"] = {
    "taskCount": len(plan["tasks"]),
    "layers": nlayers,
    "criticalPath": cpath,
    "criticalPathLength": clen,
    "review": "none",
    "mode": "EXPANSION",
}
plan["dependencies"] = {t["id"]: t["dependsOn"] for t in plan["tasks"]}
...
def md() -> str:
    ...
    a("")
    a(f"**Mode:** {t['mode']}  ")
    a(f"**Review default:** {t['review']}")
    a("")
    a(t["scopeChallenge"]["overlap"])
    a("")
    for d in t["scopeChallenge"]["decisions"]:
        a(f"- {d}")
    a("")
    a("## Prerequisites")
    a("")
    ...
    a("")
    for task in t["tasks"]:
        a(f"### {task['id']}: {task['title']}")
        a("")
        a(task["description"])
        a("")
        a(f"**Type:** {task['type']}  ")
        a(f"**Priority:** {task['priority']}  ")
        a(f"**Effort:** {task['effort']}  ")
        a(f"**Agent:** {task['agent']}  ")
        deps = task["dependsOn"]
        a(f"**Depends on:** {', '.join(deps) if deps else 'none'}  ")
        a(f"**Review:** {task['review']}")
        a("")
        a("**writeScope:**")
        a("")
        for f in task["writeScope"]:
            a(f"- `{f}`")
        a("")
        a(f"**validateCommand:** `{task['validateCommand']}`")
        a("")
        a("**Acceptance Criteria:**")
        a("")
        for c in task["acceptanceCriteria"]:
            a(f"- {c}")
        a("")
    a("## Failure Modes")
    a("")
    for p in t["failureModes"]:
        a(f"- {p}")
    a("")
    a("## Ship Cut")
    a("")
    ...
    a("## Test Coverage Map")
    a("")
    for p in t["testCoverage"]:
        a(f"- {p}")
    a("")
    a("## Execution Summary")
    a("")
    a(f"- Tasks: {t['execution']['taskCount']}")
    a(f"- Layers: {t['execution']['layers']}")
    a(f"- Critical path ({t['execution']['criticalPathLength']}): {' → '.join(cpath)}")
    a(f"- Review default: {t['review']}")
    a("")
    a("## Task Dependencies")
    a("")
    ...
    a("| --- | --- |")
    for task in t["tasks"]:
        a(f"| {task['id']} | {', '.join(task['dependsOn']) if task['dependsOn'] else '—'} |")
    a("")
    return "\n".join(lines)
...
print(f"tasks={len(plan['tasks'])} layers={nlayers} critical={' > '.join(cpath)}")

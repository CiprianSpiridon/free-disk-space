...
func TestHelpGuide(t *testing.T) {
	...
	s := buf.String()
	for _, w := range []string{"catalog add", "unassign", "--glob", "scans disable", "scans add", "--mode", "--json", "delete", "never", "node_modules", "/tmp", "/private/tmp", "kensi", "git-tracked", "skill install", "Expected time", "Do not kill", "MUST update the catalog"} {
